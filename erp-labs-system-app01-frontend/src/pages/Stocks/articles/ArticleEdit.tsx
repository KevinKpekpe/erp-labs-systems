...
export default function ArticleEdit() {
  ...
  useEffect(() => { let mounted = true; (async () => { try { const res = await apiFetch<unknown>(`/v1/stock/articles/${id}`, { method: 'GET' }, 'company'); const root = (res as { data?: unknown })?.data ?? res; if (mounted && isObject(root)) { setForm({ nom_article: String((root as any).nom_article ?? ''), categorie_id: String((root as any).categorie_id ?? ''), prix_unitaire: String((root as any).prix_unitaire ?? ''), unite_mesure: String((root as any).unite_mesure ?? ''), fournisseur: String((root as any).fournisseur ?? ''), description: String((root as any).description ?? '') }); } } catch (err) { if (mounted) setApiError(getErrorMessage(err)); } finally { if (mounted) setLoading(false); } })(); return () => { mounted = false; }; }, [id]);
  ...
  const submit = async (ev: React.FormEvent) => { ev.preventDefault(); setApiError(null); if (!validate()) return; setSubmitting(true); try { await apiFetch(`/v1/stock/articles/${id}`, { method: 'PUT', body: JSON.stringify({ nom_article: form.nom_article, categorie_id: Number(form.categorie_id), prix_unitaire: Number(form.prix_unitaire), unite_mesure: form.unite_mesure, fournisseur: form.fournisseur || undefined, description: form.description || undefined }) }, 'company'); navigate('/stocks/articles', { state: { success: 'Article modifié avec succès.' } }); } catch (err) { setApiError(getErrorMessage(err)); } finally { setSubmitting(false); } };
  ...
  return (
    <>
      <Helmet><title>Modifier article | ClinLab ERP</title></Helmet>
      <div className="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-title-md2 font-semibold text-black dark:text-white">Modifier article</h2>
          </div>
        </div>
