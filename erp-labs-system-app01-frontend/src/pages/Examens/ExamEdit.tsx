...
export default function ExamEdit() {
  ...
  useEffect(() => { let mounted = true; (async () => { try { const res = await apiFetch<unknown>(`/v1/exams/${id}`, { method: 'GET' }, 'company'); const root = (res as { data?: unknown })?.data ?? res; if (mounted && isObject(root)) {
        setForm({ nom_examen: String((root as any).nom_examen ?? ''), description: String((root as any).description ?? ''), prix: String((root as any).prix ?? ''), delai_rendu_estime: String((root as any).delai_rendu_estime ?? ''), unites_mesure: String((root as any).unites_mesure ?? ''), valeurs_reference: String((root as any).valeurs_reference ?? ''), type_echantillon: String((root as any).type_echantillon ?? ''), conditions_pre_analytiques: String((root as any).conditions_pre_analytiques ?? ''), equipement_reactifs_necessaires: String((root as any).equipement_reactifs_necessaires ?? '') });
        const arts = Array.isArray((root as any).articles) ? ((root as any).articles as any[]).filter(isObject) : [];
        if (arts.length > 0) {
          setLignes(arts.map((a: any) => ({ article_id: String(a.id ?? ''), quantite_utilisee: String(a.quantite_utilisee ?? '') })));
        }
      } } catch (err) { if (mounted) setApiError(getErrorMessage(err)); } finally { if (mounted) setLoading(false); } })(); return () => { mounted = false; }; }, [id]);
  ...
  const validate = () => {
    ...
    lignes.forEach((l, i) => { if ((l.article_id && !l.quantite_utilisee) || (!l.article_id && l.quantite_utilisee)) { e[`ligne_${i}`] = "Sélectionnez l'article et la quantité"; } });
    setErrors(e); return Object.keys(e).length === 0;
  };

  const submit = async (ev: React.FormEvent) => { ev.preventDefault(); setApiError(null); if (!validate()) return; setSubmitting(true); try { const payload: Record<string, unknown> = { nom_examen: form.nom_examen, description: form.description || undefined, prix: Number(form.prix), delai_rendu_estime: Number(form.delai_rendu_estime), unites_mesure: form.unites_mesure, valeurs_reference: form.valeurs_reference || undefined, type_echantillon: form.type_echantillon, conditions_pre_analytiques: form.conditions_pre_analytiques || undefined, equipement_reactifs_necessaires: form.equipement_reactifs_necessaires || undefined }; const lignesNettoyees = lignes.filter(l => l.article_id && l.quantite_utilisee).map(l => ({ article_id: Number(l.article_id), quantite_utilisee: Number(l.quantite_utilisee) })); if (lignesNettoyees.length > 0) (payload as any).articles = lignesNettoyees; await apiFetch(`/v1/exams/${id}`, { method: 'PUT', body: JSON.stringify(payload) }, 'company'); navigate('/examens', { state: { success: 'Examen modifié avec succès.' } }); } catch (err) { setApiError(getErrorMessage(err)); } finally { setSubmitting(false); } };

  if (loading) { return (<div className="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10"><div className="flex items-center justify-center h-64"><div className="h-8 w-8 animate-spin rounded-full border-2 border-brand-500 border-t-transparent"></div></div></div>); }

  return (
    <>
      <Helmet><title>Modifier examen | ClinLab ERP</title></Helmet>
      <div className="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-title-md2 font-semibold text-black dark:text-white">Modifier examen</h2>
          </div>
        </div>
